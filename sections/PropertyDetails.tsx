...
export default function PropertyDetails({ imovel, url, shareEmail, }: SectionProps<typeof loader>) {
    const id = useId();
    const photos = imovel.Foto ? Object.values(imovel.Foto) : [];
    const showValorDiariaAlta = imovel.ExibirValorDiariaBaixa &&
    ...
    return (<>
      ...
      <div id={id} class="relative">
        <Slider class="carousel carousel-center w-full h-[435px]">
          {photos.map(({ Foto }, index) => (<Slider.Item index={index} class="carousel-item w-full lg:w-1/3">
              <img src={Foto} class="object-cover w-full h-full" loading={index === 0 ? "eager" : "lazy"}/>
            </Slider.Item>))}
        </Slider>

        <div class="absolute top-1/2 -translate-y-1/2 left-0 w-full flex justify-between px-[10px] lg:px-[5%]">
          <Slider.PrevButton class="bg-secondary w-[32px] h-[38px] flex justify-center items-center text-white group" disabled={false}>
            <Icon id="ChevronLeft" size={24} class="group-disabled:opacity-25"/>
          </Slider.PrevButton>

          <Slider.NextButton class="bg-secondary w-[32px] h-[38px] flex justify-center items-center text-white group" disabled={false}>
            <Icon id="ChevronLeft" size={24} class="rotate-180 group-disabled:opacity-25"/>
          </Slider.NextButton>
        </div>
      </div>
      <Slider.JS rootId={id}/>
        ...
        <div class="flex flex-col pb-[15px] border-b border-black">
          ...
          <div class="flex flex-col lg:flex-row">
            ...
            <div class="flex justify-end w-full items-center gap-5 h-[58px]">
              ...
              <label htmlFor="share_modal" className="bg-transparent outline-none border-none cursor-pointer">
              ...
              <div className="modal" role="dialog">
                <div className="modal-box w-[70%] lg:w-1/2 max-w-none pb-[30px] lg:pb-[150px] rounded-[6px] shadow-[0_5px_15px_rgba(0,0,0,0.5)]">
                  ...
                  <div class="flex justify-around w-full items-center">
                    <a href={`http://www.facebook.com/sharer/sharer.php?u=${url}&t=`} class="bg-[#0a246a] text-white rounded-full h-[35px] lg:h-max py-[6px] px-[15px] lg:py-[22px] lg:px-[33px] flex items-center justify-center" target="_blank">
                      <Icon id="FacebookLetter" width={8} height={13} class="block lg:hidden"/>
                      <Icon id="FacebookLetter" width={32} height={53} class="hidden lg:block"/>
                    </a>

                    <a href={`http://api.whatsapp.com/send?text= - ${url}`} class="bg-[#159d0d] text-white rounded-full py-[6px] px-[15px] lg:py-[22px] lg:px-[26px] h-[35px] lg:h-max flex items-center justify-center" target="_blank">
                      <Icon id="WhatsApp" width={13} height={13} class="block lg:hidden"/>
                      <Icon id="WhatsApp" width={45} height={53} class="hidden lg:block"/>
                    </a>

                    <a href={`mailto:${shareEmail}?subject=PORTO BRACUHY IMÓVEIS&body=Você visualizou um imóvel em nossa página. segue o endereço. ${url}`} class="bg-secondary text-white rounded-full py-[6px] px-[15px] lg:py-[17px] lg:px-[17px] h-[35px] lg:h-max flex items-center justify-center" target="_blank">
                      <Icon id="Envelope" width={13} height={13} class="block lg:hidden"/>
                      <Icon id="Envelope" width={52} height={53} class="hidden lg:block"/>
                    </a>

                    <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${url}`} class="bg-[#0c66c2] text-white rounded-full py-[6px] px-[15px] lg:py-[22px] lg:px-[26px] h-[35px] lg:h-max flex items-center justify-center" target="_blank">
                      <Icon id="LinkedinLetter" width={13} height={13} class="block lg:hidden"/>
                      <Icon id="LinkedinLetter" width={45} height={53} class="hidden lg:block"/>
                    </a>
                  </div>
                </div>
        ...
        <div class="flex flex-col lg:flex-row gap-[30px]">
          <div class="lg:w-3/4">
            <div class="flex flex-col pt-[30px] w-full">
              ...
              {imovel.DescricaoWeb && (<div class="text-[18.2px] font-light leading-[23.66px] mb-[30px]" dangerouslySetInnerHTML={{
                __html: imovel.DescricaoWeb.replaceAll("\r\n", "<br />"),
            }}/>)}
              <span class="text-[13px] font-medium flex items-center gap-2 lg:text-[22.1px] mb-5">
                <Icon class="text-[#ff4646] block lg:hidden" id="LocationDot" size={22}/>
                <Icon class="text-[#ff4646] hidden lg:block" id="LocationDot" size={31}/>{" "}
                {imovel.Bairro} - {imovel.Cidade}/{imovel.UF}
              </span>
              <span class="text-[19.5px] lg:text-[28.6px] font-medium flex items-center gap-2 flex-wrap">
                <Icon class="text-[#87CE74] block lg:hidden" id="DollarSign" size={22}/>
                <Icon class="text-[#87CE74] hidden lg:block" id="DollarSign" size={39}/>
                {getPrice(imovel)}
            ...
            Number(imovel.ValorCondominio) > 0 && (<span class="text-[19.5px] lg:text-[22.1px] font-bold flex items-center gap-2 text-info-content">
                      Condomínio: {stringToCurrency(imovel.ValorCondominio)}
                    </span>)}
                {imovel.ValorIptu && Number(imovel.ValorIptu) > 0 && (<span class={`text-[19.5px] lg:text-[22.1px] font-bold flex items-center gap- text-info-content ${imovel.ValorCondominio
              ...
              <div class="flex flex-col mb-[35px]">
                {imovel.ExibirValorDiariaBaixa && imovel.VlrDiariaBaixa && (<>
                    {showValorDiariaAlta && (<div class="text-[18px] p-2 border-t border-[#ddd] font-bold flex items-center gap-2 text-base-200">
              ...
              <div class="flex justify-around w-full h-12 mt-auto border border-black mb-[55px]">
                {getCaracteristicas(imovel).map(({ label, value, icon }) => (<div class="flex flex-col items-center justify-center">
                    {icon.charAt(0) !== "e" ? (<Icon id={label as AvailableIcons} size={16}/>) : (<i class={`font-icomoon icomoon-${label} not-italic font-black`}></i>)}
              ...
              <div className="collapse collapse-plus bg-transparent mb-[70px]">
                ...
                <div className="collapse-content p-0">
                  <div class="flex gap-[15px] flex-wrap pb-5 shadow-[0px_12px_8px_-16px_#111]">
                    {getAdditionalCaracteristics(imovel).map((carac) => (<div class="flex justify-center items-center bg-[#ddd] rounded-[25px] py-[5px] px-5 text-[#383939] font-normal text-[13px] lg:text-[15.6px] font-slab">
                        {carac}
                      </div>))}
                  </div>
                </div>
              ...
              <div className="collapse collapse-plus bg-transparent mb-[70px]">
                ...
                <div className="collapse-content p-0">
                  <div class="flex gap-[15px] flex-wrap pb-5 shadow-[0px_12px_8px_-16px_#111]">
                    {getAdditionalInfras(imovel).map((infra) => (<div class="flex justify-center items-center bg-[#ddd] rounded-[25px] py-[5px] px-5 text-[#383939] font-normal text-[13px] lg:text-[15.6px] font-slab">
                        {infra}
                      </div>))}
                  </div>
                </div>

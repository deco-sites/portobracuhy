...
export interface Props {
    ...
    filters?: {
        ...
        Status?: "Alugado Imobiliária" | "Alugado Terceiros" | "Aluguel" | "Aluguel e Aluguel Temporada" | "Aluguel temporada" | "Pendente" | "Suspenso" | "Trabalho Interno" | "Venda" | "Venda Aluguel e Aluguel Temporada" | "Venda e Aluguel" | "Venda e Aluguel Temporada" | "Vendido Imobiliária" | "Vendido Terceiros";
        Exclusivo?: "Sim" | "Nao";
    };
    order?: {
        Bairro?: "ASC" | "DESC";
...
export default function Shelf({ title, imoveis, showAllImagesSlider = false, }: SectionProps<typeof loader>) {
    const id = useId();
    if (!imoveis || imoveis.length === 0) {
        return (<div class="flex flex-col w-[90%] mx-auto bg-[#F2F2F2] lg:bg-base-100 pt-[15px] pb-[5px] my-[25px]">
      ...
      <div class="flex flex-col w-[90%] mx-auto bg-[#F2F2F2] lg:bg-base-100 pt-[15px] pb-[5px] my-[25px]">
        ...
        <div id={id} class="relative">
          <Slider class="carousel carousel-center sm:carousel-end gap-5 sm:gap-10 w-full px-[10px] pb-[5px] lg:px-0 lg:gap-0 lg:-mx-[15px]">
            {imoveis?.map((imovel, index) => (<Slider.Item index={index} class={clx("carousel-item", "w-full box-border lg:w-1/3 lg:px-[15px]")}>
                <ShelfItem showAllImagesSlider={showAllImagesSlider} imovel={imovel}/>
              </Slider.Item>))}
          </Slider>

          <div class="w-full flex justify-between absolute z-10 right-0 top-[40%]">
            <Slider.PrevButton class={clx("hidden sm:flex justify-center items-center", "absolute -left-[45px]", "border-2 border-black", "w-[41.5px] h-[45.59px] rounded-[30px]", "hover:bg-black hover:text-white transition-colors cursor-pointer")}>
              <Icon id="ChevronLeft" width={7.5} height={13}/>
            </Slider.PrevButton>

            <Slider.NextButton class={clx("hidden sm:flex justify-center items-center", "absolute -right-[15px]", "border-2 border-black", "w-[41.5px] h-[45.59px] rounded-[30px]", "hover:bg-black hover:text-white transition-colors cursor-pointer")}>
...
async function getImoveisWithAllImages(imoveis: Imovel[], ctx: AppContext): Promise<Imovel[]> {
    const fields = [{ Foto: ["Foto", "FotoPequena", "Destaque"] }];
    const urlBase = `${ctx.loft.baseUrl}/imoveis/detalhes?key=${ctx.loft.apiKey.get()}&pesquisa={"fields":${JSON.stringify(fields)}}`;
    try {
        const imoveisComImagens = await Promise.all(imoveis.map(async (imovel) => {
            ...
            if (!response.ok) {
                throw new Error(`Failed to fetch details for imovel ${imovel.Codigo} - ${response.status} - ${response.statusText}`);
            }

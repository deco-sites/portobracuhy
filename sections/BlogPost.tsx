...
export default function BlogPost({ page, url, shareEmail, }: SectionProps<typeof loader>) {
    const { title, image, date, content, categories } = page?.post || DEFAULT_PROPS;
    ...
    return (<>
      ...
      <Image className="w-full object-cover aspect-video max-h-[500px]" width={600} src={image || ""}/>
      <div className="w-full flex flex-col gap-20 container mx-auto px-4 md:px-0 py-12 lg:py-28">
        <div className="w-full flex flex-col gap-5 lg:mx-auto text-center">
          ...
          <div class="border-t-2 border-[#afc0c7] py-[50px] flex flex-wrap gap-2 justify-center items-center w-full max-w-[400px] mx-auto">
            {categories?.map((category) => (<div class="badge px-[25px] py-[3px] bg-[#DDD] text-sm border-none h-auto leading-[1.8] ">
                {category.name}
              </div>))}
          </div>
        </div>

        <div id="content" class={CONTENT_STYLES} dangerouslySetInnerHTML={{
            __html: content,
        }}></div>
      </div>

...
export interface Props {
  ...
  /** @description Use esse campo se desejar forçar campos de busca e ignorar os que vem de retorno do CRM */
  forceFieldsContent?: forceFieldsContent;
}
...
function RegularSearch({
  ...
  const getBairrosOptions = (): string[] => {
    if (forceFieldsContent && forceFieldsContent.cidades.length > 0) {
      if (selectedCidades.length > 0) {
        return forceFieldsContent.cidades
          .filter((item) => selectedCidades.includes(item.label))
          .map((item) => item.bairros)
          .flat();
      }
      return forceFieldsContent.cidades.map((cidade) => cidade.bairros).flat();
    }
    return bairros;
  };
  ...
  return (
    <div class="flex flex-col pb-[3px]">
      ...
      <div class="flex flex-col lg:flex-row lg:gap-[3px]">
        <SearchSelect
          label={placeholders.campoCidade}
          options={
            forceFieldsContent.cidades.length > 0
              ? forceFieldsContent.cidades.map((cidade) => cidade.label)
              : cidades
          }
        ...
        />
        <SearchSelect
          label={placeholders.campoBairro}
          options={getBairrosOptions()}
        ...
        />
        <SearchSelect
          label={placeholders.campoFinalidade}
          options={categorias}
          barClass="cursor-pointer mb-[3px]"
          searchable
...
export default function BuscaImovel({
  ...
  placeholders = {
    campoCidade: "Cidades",
    campoBairro: "Condomínios",
    campoFinalidade: "Tipos de imóvel",
  },
  forceFieldsContent = { cidades: [] },
